...
import { Brand, createBrand, updateBrand, fetchBrandById } from "../constants";
...
const CreateBrandModal: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    setLogoPreview(editItem.logo || null);

    const itemId = (editItem as any)._id || editItem.id;
    fetchBrandById(itemId, token)
      .then((data: any) => {
        if (!data) return;
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (finalLogoUrl) body.logo = finalLogoUrl;
        if (slugEn.trim()) body.slug = slugEn.trim();
        if (slugAr.trim()) body.slugAr = slugAr.trim();

        if (editItem) {
          await updateBrand((editItem as any)._id || editItem.id, body, token);
          toast.success(t("savedSuccessfully") || "Saved");
        } else {
          await createBrand(body, token);
  ...
  return (
    <ModalWrapper
      ...
      title={isEditing ? `${t("edit")} ${t("brands")}` : t("createBrand")}
      titleId="create-brand-title"
      footer={
    ...
    >
      <form id="brand-form" onSubmit={handleSubmit} className="space-y-5">
        <div>
          ...
          >
            {t("brandNameEn")} <span className="text-rose-500">*</span>
          </label>
          <input
            id="brand-name-en"
            type="text"
        ...
        <div>
          <label
            ...
            className="mb-1.5 block text-sm font-semibold text-gray-700"
          >
            {t("brandNameAr")} <span className="text-rose-500">*</span>
          </label>
          <input
            id="brand-name-ar"
            type="text"
