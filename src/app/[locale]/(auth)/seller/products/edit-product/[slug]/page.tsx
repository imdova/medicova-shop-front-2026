...
import { products } from "@/data"; // Assuming you have a products constant
...
const HealthStatus = ({ product }: { product: ProductDetails }) => {
  ...
  const t = (key: { en: string; ar: string }): string => key[locale];
  ...
  useEffect(() => {
    ...
    if (foundProduct) {
      ...
      const editableProduct: ProductDetails = {
        ...
        title: foundProduct.title[locale],
        price: foundProduct.price,
        del_price: foundProduct.del_price,
        description: foundProduct.description[locale],
        features: foundProduct.features[locale],
        highlights: foundProduct.highlights[locale],
        deliveryTime: foundProduct.deliveryTime?.[locale],
        weightKg: foundProduct.weightKg,
        stock: foundProduct.stock,
        sizes: foundProduct.sizes?.map((s) => s),
        colors: foundProduct.colors?.[locale]?.map((c) => c),
        specifications: foundProduct.specifications?.map((s) => ({
          key: s.label[locale],
          value: s.content[locale],
        })),
        ...
        <div className="flex gap-2">
          ...
          <div>
            <h2 className="font-semibold">
              {product.brand?.name[locale] || t.product}
            </h2>
            <span className="text-sm">
              {product.category?.title[locale] || t.category}
            </span>
          </div>
        </div>
        ...
        <div className="col-span-1 rounded-xl border border-gray-200 bg-white p-6 lg:col-span-6">
          ...
          <div className="mb-6">
            <div className="mb-2 flex">
              <input
                type="text"
                className={`flex-grow border border-gray-300 p-2 focus:outline-none ${locale === "ar" ? "rounded-r-md" : "rounded-l-md"}`}
              ...
              <button
                type="button"
                className={`rounded-md bg-green-600 px-3 text-white hover:bg-green-700 ${locale === "ar" ? "rounded-l-md" : "rounded-r-md"}`}
          ...
          <div className="mb-6">
            <div className="mb-2 flex flex-wrap gap-2">
              <input
                type="text"
                className={`flex-grow rounded-md border border-gray-300 p-2 focus:outline-none ${locale === "ar" ? "rounded-r-md" : "rounded-l-md"}`}
              ...
              <input
                type="text"
                className={`flex-grow rounded-md border border-gray-300 p-2 focus:outline-none ${locale === "ar" ? "rounded-l-md" : "rounded-r-md"}`}
