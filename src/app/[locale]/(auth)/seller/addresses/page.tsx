...
import {
  ...
  setEncrypted,
  removeEncrypted,
} from "@/util/encryptedCookieStorage";
...
const AddressPage = () => {
  ...
  useEffect(() => {
    const loadAddresses = async () => {
      ...
      const parsedDefaultAddr = await getEncrypted<Address>("userAddress");
      if (parsedDefaultAddr) {
      ...
      const parsedSavedAddresses =
        await getEncrypted<Address[]>("savedAddresses");
      if (parsedSavedAddresses && parsedSavedAddresses.length > 0) {
        setSavedAddresses(
  ...
  const saveAddressesToStorage = async (
    ...
    if (newDefault) {
      await setEncrypted("userAddress", newDefault);
    } else {
      await removeEncrypted("userAddress");
    }
    await setEncrypted("savedAddresses", newSaved);
  ...
  const handleAddressSelectedFromModal = (address: Address) => {
    ...
    saveAddressesToStorage(finalDefault, finalSaved);
  ...
  const setAsDefault = (address: Address) => {
    ...
    saveAddressesToStorage(newDefault, newSaved);
  };
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-700">
        {translations.myAddresses[locale]}
      </h1>

      {/* Default Address Section */}
      <div
        className={`mb-8 ${locale === "ar" ? "text-right" : ""}`}
        dir={locale === "ar" ? "rtl" : "ltr"}
      >
        <h2 className="mb-2 text-sm font-semibold">
          {translations.defaultAddress[locale]}
        </h2>

        {defaultAddress ? (
          <div className="relative rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
            <div className="mb-2 flex items-start">
              <div className={`mt-1 ${locale === "ar" ? "ml-3" : "mr-3"}`}>
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  ...
                  <span className="rounded-full bg-green-100 px-2 py-1 text-xs text-green-800">
                    {translations.defaultTag[locale]}
                  </span>
                </div>
            ...
            <div
              className={`mt-2 flex ${locale === "ar" ? "justify-start space-x-reverse" : "justify-end"} space-x-2`}
            >
              <button
                ...
                title={translations.edit[locale]}
              >
                <Edit2 size={15} />
              </button>
              <button
                ...
                title={translations.delete[locale]}
              >
                <Trash2 size={15} />
              </button>
            </div>
      ...
      <div className="mb-8">
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-semibold">
            {translations.savedAddresses[locale]}
          </h2>
          <button
            onClick={() => {
          ...
          >
            ...
            <span className="hidden md:block">
              {translations.addNewAddress[locale]}
            </span>
          </button>
        </div>
              ...
              >
                ...
                <div className="mt-2 flex justify-end gap-3">
                  <button
                    ...
                    className="flex h-7 w-7 items-center justify-center rounded-lg border border-gray-300 text-green-600 hover:text-green-800"
                    title={translations.setAsDefault[locale]}
                  >
                    <Check size={15} />
                  </button>
                  <button
