...
const SwipeableNav = ({ links, locale }: SwipeableNavProps) => {
  ...
  return (
    <div className="relative bg-[#E8EFEA] shadow-sm" ref={containerRef}>
      {showLeftArrow && (
        <button
          onClick={() => scrollNav("left")}
          className="absolute left-0 top-1/2 z-10 flex h-8 w-14 -translate-y-1/2 items-center justify-center bg-gradient-to-r from-[#E8EFEA] via-[#E8EFEA] to-transparent"
          ...
          <nav className="flex min-w-full items-center justify-start px-4">
            {links.map((link, index) => (
              <div className="group shrink-0" key={index}>
                <Link
                  href={link.url}
                  className="block whitespace-nowrap px-4 py-2 text-sm font-semibold capitalize text-gray-700 hover:text-[#296D46]"
                >
                  {link.title[locale]}
                </Link>
      ...
      {showRightArrow && (
        <button
          ...
          className="absolute right-0 top-1/2 z-10 flex h-8 w-14 -translate-y-1/2 items-center justify-center bg-gradient-to-l from-[#E8EFEA] via-[#E8EFEA] to-transparent"
