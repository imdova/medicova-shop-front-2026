...
export const EnhancedDynamicSearchMenu = ({
  ...
  }, [searchTerm, allItems, activeSubmenu, items, locale]);
  ...
  const getBreadcrumbPath = useCallback((): string[] => {
    ...
    while (current) {
      path.unshift(current.label[locale] || current.label.en || "");
      if (current.parentId) {
    ...
    return path;
  }, [activeSubmenu, items, findItemById, locale]);
  ...
  return (
    <div ref={containerRef} className={cn("relative w-full", className)}>
      ...
      >
        ...
        >
          {selectedItem
            ? selectedItem.label[locale] || selectedItem.label.en || ""
