...
const WishlistCard: React.FC<WishlistCardProps> = ({
  ...
  const nudgeCount = product.nudges
    ? product.nudges[locale as keyof typeof product.nudges].length
    : 0;
          ...
          <div className="flex h-full flex-col">
              ...
              <div className="p-1">
                ...
                <div className="m-2 flex flex-wrap items-center gap-1 text-sm sm:gap-3">
                  ...
                  <del className="text-sm text-gray-600">
                    {product.del_price?.toLocaleString()} {tCommon("currency")}
                  </del>
                  {product.sale && (
                    <span className="text-light-primary text-xs font-bold">
                      {product.sale}
                    </span>
                  )}
                </div>
                ...
                <div className="relative mt-1 h-6 overflow-hidden">
                  ...
                  >
                    {product.nudges?.[
                      locale as keyof typeof product.nudges
                    ]?.map((nudge: string, index: number) => (
                      <div
                        key={index}
              ...
              {product.shippingMethod && (
                <div className="mt-2 flex items-center text-xs font-semibold">
                  <span className="bg-light-primary rounded px-2 py-1 text-white">
                    {
                      product.shippingMethod?.[
                        locale as keyof typeof product.shippingMethod
                      ]
                    }
                  </span>
                </div>
            ...
            <div className="mt-2 flex flex-col gap-2 sm:flex-row">
              <button
                ...
                onClick={(e) => {
                  e.preventDefault();
                  // potentially use router.push here if button is preferred over Link
                }}
              >
                {tProduct("viewOptions")}
              </button>
