...
import { getEncrypted, setEncrypted } from "@/util/encryptedCookieStorage";
...
export default function DeliverToModal({
  ...
  useEffect(() => {
    const loadAddresses = async () => {
      const parsed = await getEncrypted<Address[]>("savedAddresses");
      if (parsed && parsed.length > 0) {
        ...
        setSelectedAddress(defaultAddress);
        saveAddressesToStorage([defaultAddress]);
      }
    };
    loadAddresses();
  }, [currentAddress]);
  ...
  const saveAddressesToStorage = (addresses: Address[]) => {
    setEncrypted("savedAddresses", addresses);
  };
